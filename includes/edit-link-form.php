...
if ( ! empty( $link_id ) ) {
	...
	$heading      = sprintf( __( '<a href="%s">Links</a> / Edit Link', 'default' ), 'link-manager.php' );
	$submit_text  = __( 'Update Link' );
	$form_name    = 'editlink';
	$nonce_action = 'update-bookmark_' . $link_id;
} else {
	/* translators: %s: URL to Links screen. */
	$heading      = sprintf( __( '<a href="%s">Links</a> / Add New Link', 'default' ), 'link-manager.php' );
	$submit_text  = __( 'Add Link', 'default' );
	$form_name    = 'addlink';
...
get_current_screen()->add_help_tab(
	array(
		...
		'title'   => __( 'Overview' ),
		'content' =>
		'<p>' . __( 'You can add or edit bookmarks on this screen by entering information in each of the boxes. Only the link&#8217;s web address and title (the text you want to display on your site as the link) are required fields.', 'bookmark-links' ) . '</p>' .
		'<p>' . __( 'The boxes for link name, web address, and description have fixed positions, while the others may be repositioned using drag and drop. You can also hide boxes you don&#8217;t use in the Screen Options tab, or minimize boxes by clicking on the title bar of the box.', 'default' ) . '</p>' .
		'<p>' . __( 'XFN stands for <a href="https://gmpg.org/xfn/">XHTML Friends Network</a>, which is optional. WordPress allows the generation of XFN attributes to show how you are related to the authors/owners of the site to which you are linking.', 'default' ) . '</p>',
	)
);

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __( 'For more information:', 'default' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://codex.wordpress.org/Links_Add_New_Screen">Documentation on Creating Links</a>', 'default' ) . '</p>' .
	'<p>' . __( '<a href="https://wordpress.org/support/">Support</a>', 'default' ) . '</p>'
);
...
<a href="link-add.php" class="page-title-action"><?php echo esc_html_x( 'Add New', 'link', 'default' ); ?></a>

<hr class="wp-header-end">

<?php if ( isset( $_GET['added'] ) ) : ?>
<div id="message" class="updated notice is-dismissible"><p><?php _e( 'Link added.', 'default' ); ?></p></div>
<?php endif; ?>
...
<h2 class="postbox-header"><label for="link_url"><?php _e( 'Web Address', 'default' ); ?></label></h2>
<div class="inside">
